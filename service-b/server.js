...
app.get("/api/post/:id", async (req, res) => {
  try {
    ...
    const [postResponse, commentsResponse] = await Promise.all([
      ...
      Promise.resolve({ data: [] }), // TODO-add this back before making PR, mark this as unintended deviation if Tusk Drift API test fails
    ]);
...
const main = async () => {
  app.listen(PORT, () => {
    ...
    console.log(
      "  GET /api/random-user      - Fetches random user from external API",
    );
    console.log(
      "  GET /api/weather-activity - Multiple API calls with business logic",
    );
